...
class DeformableTransformer_Det(nn.Module):
    ...
    def init_control_points_from_anchor(self, reference_points_anchor):
        ...
        # 设置检测框右边的6-8点
        # reference_points[:, :, pts_per_side : 2 * pts_per_side - 1, 1].add_(
        #     reference_points[:, :, pts_per_side : 2 * pts_per_side - 1, 2] / 2)
        reference_points[:, :, pts_per_side - 1: 2 * pts_per_side - 1, 1] = reference_points[:, :,
                                                                            pts_per_side - 1: 2 * pts_per_side - 1,
                                                                            3] / (pts_per_side - 1)
        reference_points[:, :, pts_per_side - 1: 2 * pts_per_side - 1, 1] = torch.cumsum(
            reference_points[:, :, pts_per_side - 1: 2 * pts_per_side - 1, 1], dim=-1)
        reference_points[:, :, pts_per_side - 1: 2 * pts_per_side - 1, 0].add_(
            reference_points[:, :, pts_per_side - 1: 2 * pts_per_side - 1, 2] / 2)

        # 设置检测框下边的9-13点
        # reference_points[:,:,2 * pts_per_side-1: 3 * pts_per_side -2, 0].add_(
        #     reference_points[:,:,2 * pts_per_side  : 3 * pts_per_side - 2, 2] / 2)
        reference_points[:, :, 2 * pts_per_side - 1: 3 * pts_per_side - 2, 0] = reference_points[:, :,
                                                                                2 * pts_per_side - 1: 3 * pts_per_side - 2,
                                                                                2] / (pts_per_side - 1)
        reference_points[:, :, 2 * pts_per_side - 1: 3 * pts_per_side - 2, 0] = torch.cumsum(
            reference_points[:, :, 2 * pts_per_side - 1: 3 * pts_per_side - 2, 0], dim=-1)
        reference_points[:, :, 2 * pts_per_side - 1: 3 * pts_per_side - 2, 1].add_(
            reference_points[:, :, 2 * pts_per_side - 1: 3 * pts_per_side - 2, 3] / 2)

        # 设置检测框左边的14-16点
        # reference_points[:, :, 3 * pts_per_side - 2: 3 * pts_per_side + 1, 1].sub_(
        #     reference_points[:, :, 3 * pts_per_side - 2: 3 * pts_per_side + 1, 2] / 2)
        reference_points[:, :, 3 * pts_per_side - 2: 3 * pts_per_side + 1, 1] = reference_points[:, :,
                                                                                3 * pts_per_side - 2: 3 * pts_per_side + 1,
                                                                                3] / (pts_per_side - 1)
        reference_points[:, :, 3 * pts_per_side - 2: 3 * pts_per_side + 1, 1] = torch.cumsum(
            reference_points[:, :, 3 * pts_per_side - 2: 3 * pts_per_side + 1, 1], dim=-1)
        reference_points[:, :, 3 * pts_per_side - 2: 3 * pts_per_side + 1, 0].add_(
            reference_points[:, :, 3 * pts_per_side - 2: 3 * pts_per_side + 1, 2] / 2)
